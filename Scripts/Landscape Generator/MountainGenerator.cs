...
public class MountainGenerator : MonoBehaviour
{
    ...
    public Tile[] mount33Tile;
    public Tile[] mount22Tile;
    public float mountain33Chance;
    public float mountain22Chance;
    ...
    public void GenerateSingle()
    {
        for (int x = 0; x < mapSize.x; x++)
            for (int y = 0; y < mapSize.y; y++)
                if (Random.value < mountain33Chance && mapManager.GetMapFullness(new Vector2Int(3, 3), new Vector2(x, y)))
                {
                    mapManager.SetMapFullness(new Vector2Int(3, 3), new Vector2(x, y), false);
                    tilemap.SetTile(new Vector3Int(x, y, 0), mount33Tile[Random.Range(0, mount33Tile.Length - 1)]);
                }
                else
                if (Random.value < mountain22Chance && mapManager.GetMapFullness(new Vector2Int(2, 2), new Vector2(x + 0.5f, y + 0.5f)))
                {
                    mapManager.SetMapFullness(new Vector2Int(2, 2), new Vector2(x + 0.5f, y + 0.5f), false);
                    tilemap.SetTile(new Vector3Int(x, y, 0), mount22Tile[Random.Range(0, mount22Tile.Length - 1)]);
                }
    ...
    private void SetTile(int x, int y)
    {
        ...
        map[x, y] = true;
        mapManager.SetMapFullness(Vector2Int.one, new Vector2(x, y), index == 15);
